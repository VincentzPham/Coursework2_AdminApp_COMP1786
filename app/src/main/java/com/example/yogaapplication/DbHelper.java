...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public boolean hasYogaClasses(String courseId) {
        SQLiteDatabase db = this.getReadableDatabase(); // Mở cơ sở dữ liệu để đọc
        Cursor cursor = db.rawQuery("SELECT COUNT(*) FROM " + TABLE_CLASS + " WHERE " + COLUMN_CLASS_COURSE_ID + "=?", new String[]{courseId});
        if (cursor != null) {
            cursor.moveToFirst(); // Di chuyển tới dòng đầu tiên của kết quả
            int count = cursor.getInt(0); // Lấy số lượng lớp học
            cursor.close(); // Đóng con trỏ để tránh rò rỉ bộ nhớ
            return count > 0; // Trả về true nếu có ít nhất một lớp con
        }
        return false; // Trả về false nếu không có lớp học nào
    }
}
