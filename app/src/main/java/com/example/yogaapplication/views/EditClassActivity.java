...
public class EditClassActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize date format with time zone
        ...
        if (classId != null) {
            ...
            buttonDelete.setOnClickListener(view -> delete(classId));
        } else {
        ...
        buttonSave.setOnClickListener(view -> saveClass());
    ...
    private void checkFieldsForEmptyValues() {
        ...
        buttonSave.setEnabled(!name.isEmpty() && !date.isEmpty() && !teacher.isEmpty());
    }
    ...
    private void loadCourses() {
        ...
        Cursor cursor = null;
        try {
            cursor = db.query(DbHelper.TABLE_COURSE, null, null, null, null, null, null);
    ...
    private void loadClassDetails(String classId) {
        ...
        if (yogaClass != null) {
            ...
            showToast("Class details not found.");
            finish();
        }
    }
    ...
    private void saveClass() {
        ...
        Course selectedCourse = dbHelper.getCourseById(courseId);
        if (selectedCourse == null) {
            showToast("Selected course not found.");
            return;
        }
        String expectedDayOfWeek = selectedCourse.getDayOfWeek();
        ...
        if (!validateDate(dateStr, expectedDayOfWeek)) {
            errorDate.setVisibility(View.VISIBLE);
            errorDate.setText(getString(R.string.error_date_not_matching_course_day, expectedDayOfWeek));
            return;
        } else {
            errorDate.setVisibility(View.GONE);
        }
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                ContentValues values = getClassContentValues(newClass);
                ...
                if (classId != null) {
                    ...
                    if (rowsAffected > 0) {
                        runOnUiThread(() -> showToast("Class updated successfully!"));
                    } else {
                        runOnUiThread(() -> showToast("Error updating class."));
                    }
                    ...
                    if (newRowId != -1) {
                        runOnUiThread(() -> showToast("Class added successfully!"));
                    } else {
                        runOnUiThread(() -> showToast("Error adding class."));
                    }
                }
                ...
                e.printStackTrace();
                runOnUiThread(() -> showToast("An error occurred while saving the class."));
            } finally {
                if (db != null) {
                    db.close();
                }
    ...
    private ContentValues getClassContentValues(YogaClass yogaClass) {
        ContentValues values = new ContentValues();
    ...
    private boolean validate() {
        ...
        if (editName.getText().toString().trim().isEmpty()) {
            errorName.setVisibility(View.VISIBLE);
            errorName.setText(getString(R.string.error_enter_class_name));
            isValid = false;
        } else {
            errorName.setVisibility(View.GONE);
        }
        ...
        if (editDate.getText().toString().trim().isEmpty()) {
            errorDate.setVisibility(View.VISIBLE);
            errorDate.setText(getString(R.string.error_enter_date));
            isValid = false;
        } else {
            errorDate.setVisibility(View.GONE);
        }
        ...
        if (editTeacher.getText().toString().trim().isEmpty()) {
            errorTeacher.setVisibility(View.VISIBLE);
            errorTeacher.setText(getString(R.string.error_enter_teacher_name));
            isValid = false;
        } else {
            errorTeacher.setVisibility(View.GONE);
        }
    ...
    private boolean validateDate(String dateInput, String expectedDayOfWeek) {
        if (!isValidDateFormat(dateInput)) {
            errorDate.setVisibility(View.VISIBLE);
            errorDate.setText(getString(R.string.error_invalid_date_format));
            return false;
        }
        ...
        try {
            ...
            // Get day names from DateFormatSymbols
            String[] days = new DateFormatSymbols().getWeekdays();
            String actualDayOfWeek = days[dayOfWeek];
        ...
        builder.setPositiveButton("Yes", (dialog, which) -> {
            ...
            executor.execute(() -> {
                ...
                try {
                    ...
                    if (rowsDeleted > 0) {
                        runOnUiThread(() -> showToast("Class deleted successfully!"));
                    } else {
                        runOnUiThread(() -> showToast("Error deleting class."));
                    }
                    ...
                    e.printStackTrace();
                    runOnUiThread(() -> showToast("An error occurred while deleting the class."));
                } finally {
                    if (db != null) {
                        db.close();
                    }
    ...
    private void showToast(String message) {
        Toast.makeText(EditClassActivity.this, message, Toast.LENGTH_SHORT).show();
    }
}
